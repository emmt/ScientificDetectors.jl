...
while, except for $g=1$ (the photon-counting case) and there is a constant bias
...
Approximating the sum in Eq.~\eqref{eq:pdf-ADU-exact} by the mean number
of terms (that is $g$) times the median value of the terms yields:
\begin{align}
  \Pr\Paren*{d \Given \mu}
...
other.  The Gaussian approximation is however easier to manipulate.
...
electrons following a Poissonian distribution with a given mean \oops{(ref?)}.
In that case, the distribution given in Eq.~\eqref{eq:pdf-ADU-exact} is
correct provided the mean number of electrons $\mu$ or the bias $z$ accounts
for this additional contribution \oops{(explain better)}. In any case, we will
assume the Gaussian approximation of the distribution of ADU's in the
following. \oops{Justify a bit more.}
