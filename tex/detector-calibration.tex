...
\Delta t > 0$ and the only background source is the dark current.
The calibrated images are given by an affine correction of the raw images:
\begin{equation}
...
are given by:
\begin{align}
  m^\Tag{dark}_j &= c^\Tag{dark}_j\,\Delta t + z_j \pm ... \,,
  \label{eq:Themis-dark-avg} \\
  v^\Tag{dark}_j &= c^\Tag{dark}_j\,\Delta t/g_j + \sigma^2_j \pm ... \,.
...
\begin{align}
  m^\Tag{lamp}_j
  &= c^\Tag{lamp}_j\,\Delta t + z_j \pm ... \,,
  \label{eq:Themis-led-avg} \\
  v^\Tag{lamp}_j
  &= c^\Tag{lamp}_j\,\Delta t/g_j + \sigma^2_j \pm ... \,.
  \label{eq:Themis-led-var} \\
  &\approx \Paren[\big]{m^\Tag{lamp}_j - b_j}/g_j + \sigma^2_j
...
\begin{align}
  \varphi_j(\V\theta_j)
  &= N^\Tag{dark}\,\Paren*{
    \frac{v^\Tag{dark}_j + \Paren*{m^\Tag{dark}_j - z_j}^2}{\sigma^2_j}
    + \log\sigma^2_j
  } \notag\\
  &\quad+ N^\Tag{lamp}\,\Paren*{
    \frac{v^\Tag{lamp}_j + \Paren*{m^\Tag{lamp}_j - z_j - x_j}^2}
         {x_j/g_j + \sigma^2_j}
    + \log\Paren*{x_j/g_j + \sigma^2_j}
  } \notag\\
  &= g_j\,\Paren*{
    N^\Tag{dark}\,\frac{v^\Tag{dark}_j + \Paren*{m^\Tag{dark}_j - z_j}^2}
                       {u_j} +
    N^\Tag{lamp}\,\frac{v^\Tag{lamp}_j + \Paren*{m^\Tag{lamp}_j - z_j - x_j}^2}
                      {u_j + x_j}
  } \notag\\
  &\quad - \Paren*{N^\Tag{dark} + N^\Tag{lamp}}\log g_j
    + N^\Tag{dark}\,\log u_j
    + N^\Tag{lamp}\,\log\Paren*{u_j + x_j}
\end{align}
with $x_j = c^\Tag{lamp}_j\,\Delta t$ and $u_j = g_j\,\sigma^2_j$ as before
but with $\V\theta = (z_j, g_j, u_j, x_j)$ this time.

It appears that conventional calibration amounts to first maximize the
likelihood of the dark images in the bias $z_j$ and the readout noise
$\sigma_j$ and then maximize the likelihood of the led images in the remaining
detector parameters.

The best bias does not depend on the gain:
\begin{align}
  z^+_j(u,x)
  &\bydef \argmin_{z} \varphi_j(z,g,u,x) \notag\\
  &= \frac{
    \frac{N^\Tag{dark}\,m^\Tag{dark}_j}{u} +
    \frac{N^\Tag{lamp}\,\Paren*{m^\Tag{lamp}_j - xj}}{u + x}
  }{
    \frac{N^\Tag{dark}}{u} +
    \frac{N^\Tag{lamp}}{u + x}
  } \, .
...
\begin{align}
  g^+_j(u,x)
  &\bydef \argmin_{g > 0} \Brace*{\min_{z} \varphi_j(z,g,u,x)} \notag\\
  &= \frac{
    N^\Tag{dark} + N^\Tag{lamp}
  }{
    N^\Tag{dark}\,\frac{
      v^\Tag{dark}_j + \Paren*{m^\Tag{dark}_j - z^+_j(u,x)}^2
    }{
      u
    } + N^\Tag{lamp}\,\frac{
      v^\Tag{lamp}_j + \Paren*{m^\Tag{lamp}_j - z^+_j(u,x) - x}^2
    }{
      u + x
    }
  } \, .
...
\begin{align}
  \varphi^+_j(u, x)
  &\bydef \min_{z,g>0} \varphi_j(z,g,u,x) \notag \\
  &= \varphi_j\Paren*{z^+_j(u,x),g^+_j(u,x),u,x} \notag \\
  &= \Paren*{N^\Tag{dark} + N^\Tag{lamp}}\,\Paren*{1 - \log g_j^+(u,x)}
  \notag \\
  &\quad
    + N^\Tag{dark}\,\log u
    + N^\Tag{lamp}\,\log\Paren*{u + x} \, ,
\end{align}
and we minimize this function in $(u,x)$ under the constraints $u>0$ and $x≥0$.
To apply VMLM-B, the following partial derivatives are needed:
\begin{align}
  \PDer{\varphi^+_j(u, x)}{u}
  &= \frac{N^\Tag{dark}}{u} - N^\Tag{dark}\,g^ +_j(u,x)\,\frac{
      v^\Tag{dark}_j + \Paren*{m^\Tag{dark}_j - z^+_j(u,x)}^2
    }{
      u^2
    } \notag\\
  &\quad + \frac{N^\Tag{lamp}}{u + x} - N^\Tag{lamp}\,g^+_j(u,x)\,\frac{
      v^\Tag{lamp}_j + \Paren*{m^\Tag{lamp}_j - z^+_j(u,x) - x}^2
    }{
      \Paren*{u + x}^2
    }
    \, , \\
  \PDer{\varphi^+_j(u, x)}{x}
  &= 2\,N^\Tag{lamp}\,g^+_j(u,x)\,\frac{
       z^+_j(u,x) + x - m^\Tag{lamp}_j
    }{
      u + x
    } \notag\\
  &\quad + \frac{N^\Tag{lamp}}{u + x} - N^\Tag{lamp}\,g^+_j(u,x)\,\frac{
