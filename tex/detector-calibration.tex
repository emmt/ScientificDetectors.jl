...
various fluxes $\mu$ and gains $g$ (the offset is $z = 0$). \label{fig:means}}
\end{figure}
...
\caption{Difference between the variance of the detector signal and $\mu/g^2$ for
various fluxes $\mu$ and gains $g$ (the offset is zero). \label{fig:variances}}
\end{figure}
...
leads to irregular distributions show by Fig.~\ref{fig:histograms} unless the
gain $g$ is integer.  Approximating the sum in Eq.~\eqref{eq:pdf-detector-model} by
the mean number of terms (that is $g$) times the median value of the terms yields:
\begin{align}
  \Pr\Paren*{d \Given \mu}
  \approx \frac{g\,\mathe^{-\mu}\,\mu^{(d - z)\,g}}{\Gamma\Paren*{(d - z)\,g + 1}}
  \label{eq:pdf-detector-approx}
\end{align}
not accounting for saturation.
...
\end{align}
There are no closed form expressions for these quantities. Without the rounding
errors due to conversion to an integer number of ADU, the expectation and
variance of the signal are given by:
\begin{align}
...
the mean and variance accounting and not accounting for rounding errors, that
is between Eq.~\eqref{eq:exact-detector-mean} and
...
Fig.~\ref{fig:variances}.  Clearly there is a vanishing bias for the estimation
of the mean given by Eq.~\eqref{eq:detector-mean-with-no-rounding-errors}
while, except for $g=1$ (the photon-counting case), there is a constant bias
for the variance given by
Eq.~\eqref{eq:detector-variance-with-no-rounding-errors}.  The variance bias
can be approximately estimated if one assume that rounding errors uniformly are
...
\caption{Distributions of probabilities of the detector levels for a mean number of electrons $\mu = 200$ and variable gains $g \sim 3$. The offsets $z$ have been chosen to separate the different distributions. \label{fig:histograms}}
\end{figure}
