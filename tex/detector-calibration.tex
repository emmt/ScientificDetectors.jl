...
various fluxes $\mu$ and gains $g$. The curves have been averaged with respect to the offset $z $. \label{fig:variances}}
\end{figure}
...
Eq.~\eqref{eq:pdf-detector-model}.  There are no closed form expressions for
these quantities. Without the rounding errors due to conversion to an integer
number of ADU, the expectation and variance of the signal are given by:
\begin{align}
...
Fig.~\ref{fig:variances}.  Clearly there is a small variable bias for the
estimation of the mean given by
...
other.  The Gaussian approximation is however easier to handle.
