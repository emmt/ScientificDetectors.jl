...
\newcommand*{\Authors}{Éric Thiébaut}
...
Let $n_j$ be the number of electrons transferred\footnote{electrons captured by
...
\begin{equation}
  d_j \approx
  \min\Paren*{\max\Paren*{0, \Round*{n_j/g_j + z_j}}, 2^{n_\Tag{bits}} - 1}
  \label{eq:conversion}
\end{equation}
where $g_j$ and $z_j$ are respectively the \emph{gain} and the \emph{bias} of
...
perturbations than rounding errors and shot noise ($n_j$ is a random variable
...
The readout noise accounts for all detector noises, including the digitization
errors.  Hence assuming rounding to the nearest ADU yields $\sigma_j ≥
1/\sqrt{12}$.  Other constraints are $c_{j,\ell} ≥ 0$ and $g_j > 0$ because the
incident light level and the dark current are nonnegative and the gain is
strictly positive.
...
\end{equation}
This expression is trivial to minimize with respect to the bias $z_j$ leading
to the maximum likelihood estimator of the bias knowing the other parameters.
Introducing $\V c_j = \Paren*{c_{j,\ell}}_{\ell=1}^L$ with $L$ the number of
considered illuminations, the maximum likelihood estimator of the bias knowing
...
multi-dimensional minimization algorithm implementing bound constraints, \eg
...
\begin{equation}
  \psi_j(\V c, u) = \sum_{k,\ell} \Paren*{
    g^+_j(\V c, u)\,w_{k,\ell}(\V c, u)\,x_{j,k,\ell}^2(\V c, u)
   - \log g^+_j(\V c, u) - \log w_{k,\ell}(\V c, u)
 } \, .
\end{equation}
The partial derivatives of the criterion are needed by VMLMB:
\begin{align}
  \PDer{\psi(\V c, u)}{c_{\ell}}
  &=\sum_{k} w_{k,\ell}(\V c, u)\,\Paren*{
    1 - g^+_j(\V c, u)\,\,x_{j,k,\ell}(\V c, u)\,
    \Paren*{2 + w_{k,\ell}(\V c, u)\,x_{j,k,\ell}(\V c, u)}
  }\,\Delta t_{k,\ell} \,,\\
  \PDer{\psi(\V c, u)}{u}
  &= \sum_{k,\ell} w_{k,\ell}(\V c, u)\,\Paren*{
    1 - g^+_j(\V c, u)\,w_{k,\ell}(\V c, u) \, x_{j,k,\ell}^2(\V c, u)
  } \,.
\end{align}
...
following approximations for the second derivatives: \oops{CHECK!!!}
\begin{align}
  \PDer{^2\psi(\V c, u)}{c_{\ell}^2}
  &= \sum_{k} w_{k,\ell}\,\Paren*{
    2\,g^+_j\,\Paren*{1 + w_{k,\ell}\,x_{j,k,\ell}}^2 - w_{k,\ell}
  }\,\Delta t_{k,\ell}^2 \notag\\
  &\approx \sum_{k} w_{k,\ell}\,\Paren*{2\,g^+_j + w_{k,\ell}}
  \,\Delta t_{k,\ell}^2 \,,\\
  \PDer{^2\psi(\V c, u)}{u^2}
  &= \sum_{k,\ell} w_{k,\ell}^2\,\Paren*{
    2\,g^+_j\,w_{k,\ell}\,x_{j,k,\ell}^2 - 1
  } - w\,\PDer{g^+_j(\V c, u)}{u}\notag\\
  &\approx \sum_{k,\ell} w_{k,\ell}^2 \,,
\end{align}
where the two approximations comes from the following approximate and exact
averaged values:
\begin{align*}
  ...
  &= 1\,,
\end{align*}
the latter coming from Eq.~\eqref{eq:best-gain}.
...
\begin{equation}
  ...
  \prod_{i=1}^{m} p_i^{n_i} \, ,
\end{equation}
where $m$ is the number of bins, $n_i$ is the number of events occurring in the $i$-th bin and $p_i$ is the probability of occurrence in this bin.

The maximum likelihood estimator $\estim{\V\theta}$ of the parameters $\V\theta$ of
the distribution $p_i(\V\theta)$ is thus given by:
\begin{equation}
  \estim{\V\theta} =
...
FITS keyword \FitsKeyword{CALIB}\FitsVariable{n} with \FitsVariable{n} an integer number specifies the identifier (a string) of the calibration source which is responsible of the \FitsVariable{n}-th \emph{current} term (in ADUs/second).  There can be from 0 to 999 such sources.  The number is equal to $\FitsKeyword{NAXIS4} - 4$.
