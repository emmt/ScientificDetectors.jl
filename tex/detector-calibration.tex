...
with Poissonian distribution).  Other possible errors or noise are: the
...
$d$ knowing the mean number of electrons is:
\begin{align}
...
to the offset $z $. \oops{where is the curve for gain = 1.0? [MT]}\label{fig:means}}
\end{figure}
...
unsaturated pixels have a linear behavior with respect to the incident flux
...
variance of the dark images are given by:
\begin{align}
...
\oops{As designed, $c^\Tag{bg}_j$ also contains $c^\Tag{dark}_j$ since
background images cannot be obtained without any contribution of the dark
current [MT].}
...
\subsection{Grouping calibration data acquired under same conditions}

For a given category $\ell$ of illumination, several frames, say $N_{k,\ell}$,
may have been acquired with the same exposure time $\Delta t_{k,\ell}$:
\begin{equation}
  d_{j,k,\ell,n} = c_{j,\ell}\,\Delta t_{k,\ell} + z_j + e_{j,k,\ell,n} \, .
\end{equation}
Each of these data frames is such that ($\forall n \in
\Brace{1,2,\ldots,N_{k,\ell}}$):
\begin{align}
  \Expect*{d_{j,k,\ell,n}} &= m_{j,k,\ell} = c_{j,\ell}\,\Delta t_{k,\ell} + z_j\\
  \Var*{d_{j,k,\ell,n}}
   &= v_{j,k,\ell}
   = \frac{c_{j,\ell}\,\Delta t_{k,\ell}}{g_j} + \sigma_j^2
\end{align}

Assuming mutually independent pixels and data frames and approximating their
statistics by a non-uniform Gaussian distribution, the co-log-likelihood of the
data set writes:
\begin{align}
  \phi &= \sum_{j} \Brace*{
    \phi_j =
    \sum_{k,\ell,n} \Paren*{
      \frac{\Paren*{d_{j,k,\ell,n} - m_{j,k,\ell}}^2}{v_{j,k,\ell}}
      + \log v_{j,k,\ell}
    }
  }
\end{align}

Introducing the empirical mean and variance of the $N_{k,\ell}$ data frames
acquired with the same exposure time and illumination:
\begin{align}
  \label{eq:sample-mean}
  \estim{m}_{j,k,\ell}
  &= \frac{1}{N_{k,\ell}} \sum_{n=1}^{N_{k,\ell}} d_{j,k,\ell,n} \, , \\
  \label{eq:sample-variance}
  \estim{v}_{j,k,\ell}
  &= \frac{1}{N_{k,\ell}}\sum_{n=1}^{N_{k,\ell}}
    \Paren*{d_{j,k,\ell,n} - \estim{m}_{j,k,\ell}}^2 \, ,
\end{align}
the pixel-wise co-log-likelihood can be simplified\footnote{replace
$\sum_n\Paren*{d_{j,k,\ell,n} - m_{j,k,\ell}}^2$ by
$\sum_n\Paren*{\Paren*{d_{j,k,\ell,n} - \estim{m}_{j,k,\ell}} +
\Paren*{\estim{m}_{j,k,\ell} - m_{j,k,\ell}}}^2$, then develop the squares and
simplify} to:
\begin{align}
  \label{eq:pixelwise-cologlikelihood}
  \phi_j =
  \sum_{k,\ell} N_{k,\ell}\times\Paren*{
    \frac{
      \estim{v}_{j,k,\ell}
      + \Paren*{\estim{m}_{j,k,\ell} - m_{j,k,\ell}}^2
    }{
      v_{j,k,\ell}
    }
    + \log v_{j,k,\ell}
...
\end{align}
Hence the empirical mean $\estim{\V m}_{k,\ell}$ and variance  $\estim{\V
v}_{k,\ell}$ provide sufficient statistics for the $N_{k,\ell}$ data frames
acquired with the same exposure time and illumination.  Note that the empirical
variance  $\estim{\V v}_{k,\ell}$ is a biased estimator of the variance (by a
factor $N_{k,\ell}/(N_{k,\ell} - 1)$).  Using this sufficient statistics and the
pixel-wise co-log-likelihood defined in Eq.~\eqref{eq:pixelwise-cologlikelihood}
can significantly reduce the computational burden and the memory requirements.

If only a single data frame has been acquired under $\ell$-th illumination and
$k$-th exposure time, then $N_{k,\ell} = 1$ and $\estim{\V m}_{k,\ell} = \V
d_{k,\ell}$ while $\estim{\V v}_{k,\ell} = \V0$.

The best bias given the other parameters is a simple weighted average:
\begin{equation}
  \boxed{
    z^+_j = \frac{
      \sum_{k,\ell}\frac{N_{k,\ell}}{v_{j,k,\ell}}\,\Paren*{
        \estim{m}_{j,k,\ell} - c_{j,\ell}\,\Delta t_{k,\ell}
      }
    }{
      \sum_{k,\ell}\frac{N_{k,\ell}}{v_{j,k,\ell}}
    }
  }
  \label{eq:best-bias}
\end{equation}
...
from the solution is of prime importance. We note that if the bias $z_j$ and
the ratio $\tau_{j,\ell} = u_j/c_{j,\ell} > 0$ are assumed known, the
\emph{current} terms $c_{j,\ell}$ are approximately solutions of simple
independent linear least squares problems:
\begin{align}
  c^+_{j,\ell}(z,\tau)
  &= \argmin_{{j,\ell}}\phi(\V\theta)
  \quad\text{s.t.\ $u_j = \tau\,c_{j,\ell}$}\quad \notag\\
  &\approx \argmin_{c}\sum_{k} \frac{
...
\end{equation}
We further impose that $\tau_{j,\ell} = \tau$ be the same for all kinds
...
\begin{align}
  m_{j,\ell}
  &\bydef \frac{1}{N_\ell} \sum_{k=1}^{N_\ell} d_{j,k,\ell} \notag\\
  &= c_{j,\ell}\,\Delta t_{\ell} + z_j
     \pm \sqrt{
       \frac{c_{j,\ell}\,\Delta t_{\ell} + g_j\,\sigma_j^2}{g_j\,N_{\ell}}
     } \, , \\
  v^\Tag{ub}_{j,\ell}
  &\bydef \frac{1}{N_\ell - 1} \sum_{k=1}^{N_{\ell}}
    \Paren*{d_{j,k,\ell} - m_{j,\ell}}^2 \notag\\
  &=  \Paren*{\frac{c_{j,\ell}\,\Delta t_{\ell}}{g_j} + \sigma_j^2} \,
     \Paren*{1 \pm \sqrt{\frac{2}{N_{\ell} - 1}}} \, , \\
  v^\Tag{ml}_{j,\ell}
  &\bydef \frac{1}{N_\ell} \sum_{k=1}^{N_{\ell}}
    \Paren*{d_{j,k,\ell} - m_{j,\ell}}^2 \notag\\
...
$\sigma_{j,\ell}^2(\V\theta) = \Var*{d_{j,k,\ell}\Given\V\theta}$ are the
expectation and variance of the data knowing the parameters which, according to
the considered conditions, do not depend on $k$.  From the definitions of the
empirical mean and variance, the numerator in the above right hand side
expression can be expanded and simplified:
\begin{align}
  \sum_{k=1}^{N_\ell} \Paren[\big]{d_{j,k,\ell} - \mu_{j,\ell}(\V\theta)}^2
  &= \sum_{k=1}^{N_\ell} d_{j,k,\ell}^2
   - 2\,\mu_{j,\ell}(\V\theta)\,\sum_{k=1}^{N_\ell} d_{j,k,\ell}
   + N_{\ell} \, \mu_{j,\ell}^2(\V\theta) \notag\\
  &= N_\ell\,\Paren*{v_{j,\ell}^\Tag{ml} + m_{j,\ell}^2}
...
\begin{equation}
  \estim{\V\theta}
  = \argmin_{\V\theta \in \Theta} \sum_{j,\ell} N_\ell\,
  \Paren*{
    \frac{
      v_{j,\ell}^\Tag{ml}
      + \Paren[\big]{m_{j,\ell} - \mu_{j,\ell}(\V\theta)}^2
...
\begin{equation}
  \estim{\V\theta}
  \approx \argmin_{\V\theta \in \Theta} \sum_{j,\ell} N_\ell\,
  \frac{
    \Paren[\big]{m_{j,\ell} - \mu_{j,\ell}(\V\theta)}^2
  }{
    v_{j,\ell}^\Tag{ml}
...
other.  \oops{(Only true if $N_\ell = N$, whatever $\ell$)}.


\subsection{A simple example: the Themis wavefront sensor}

In the case of Themis adaptive optics (AO), the wavefront sensor camera is
operated at the same exposure time for all acquisitions, $\Delta t_{k,\ell} =
\Delta t > 0$, and the only background source is the dark current.  Using the
results of Section~\ref{sec:using-sample-means-and-variances}, the calibration
procedure just requires the empirical mean and variance of three kinds of
images:
\begin{description}
\item[Dark images] are acquired with no illumination on the camera. These
images thus depend on the bias and on the dark current
...
\begin{align}
  m^\Tag{dark}_j
  &\bydef \frac{1}{N^\Tag{dark}} \,
    \sum_{k=1}^{N^\Tag{dark}} d_{j,k}^\Tag{dark}
    \notag\\
  &= c^\Tag{dark}_j\,\Delta t + z_j
    \pm \sqrt{
      \frac{
        c^\Tag{dark}_j\,\Delta t + g_j\,\sigma_j^2
      }{
        N^\Tag{dark}\,g_j
      }
    } \,,
  ...
  v^\Tag{dark}_j
  &\bydef \frac{1}{N^\Tag{dark} - 1} \,
    \sum_{k=1}^{N^\Tag{dark}} \Paren*{d_{j,k}^\Tag{dark} - m^\Tag{dark}_j}^2
    \notag\\
  &= %\frac{N^\Tag{dark}}{N^\Tag{dark} - 1}\,
    \frac{c^\Tag{dark}_j\,\Delta t + g_j\,\sigma_j^2}{g_j}\,
    \Paren*{1 \pm \sqrt{\frac{2}{N^\Tag{dark} - 1}}} \,,
    \label{eq:Themis-dark-var}
\end{align}
with $\V d^\Tag{dark}_k$ the $k$-th dark image, $N^\Tag{dark}$ the number of
such images and $\V c^\Tag{dark}$ the contribution of the dark current.

\item[Stable images] are acquired with an illumination that is stable in time.
In practice, this illumination is provided by a LED lamp. Their empirical means
and variance are given by:
\begin{align}
  m^\Tag{lamp}_j
  &\bydef \frac{1}{N^\Tag{lamp}} \,
    \sum_{k=1}^{N^\Tag{lamp}} d_{j,k}^\Tag{lamp}
    \notag\\
  &= \Paren*{c^\Tag{lamp}_j + c^\Tag{dark}_j}\,\Delta t + z_j
    \pm \sqrt{
      \frac{
        \Paren*{c^\Tag{lamp}_j + c^\Tag{dark}_j}\,\Delta t + g_j\,\sigma_j^2
      }{
        N^\Tag{lamp}\,g_j
      }
    } \,,
    \label{eq:Themis-lamp-avg} \\
  v^\Tag{dark}_j
  &\bydef \frac{1}{N^\Tag{lamp} - 1} \,
    \sum_{k=1}^{N^\Tag{lamp}} \Paren*{d_{j,k}^\Tag{lamp} - m^\Tag{lamp}_j}^2
    \notag\\
  &= %\frac{N^\Tag{lamp}}{N^\Tag{lamp} - 1}\,
    \frac{
      \Paren*{c^\Tag{lamp}_j + c^\Tag{dark}_j}\,\Delta t + g_j\,\sigma_j^2
    }{
      g_j
    }\,
    \Paren*{1 \pm \sqrt{\frac{2}{N^\Tag{lamp} - 1}}} \,,
    \label{eq:Themis-lamp-var-approx}
\end{align}
with $\V d^\Tag{lamp}_k$ the $k$-th dark image, $N^\Tag{lamp}$ the number of
such images and $\V c^\Tag{lamp}$ the contribution of the lamp only.
...
\begin{equation}
  m^\Tag{sun}_j \approx f^\Tag{flat}_j/a_j + c^\Tag{dark}_j\,\Delta t + z_j \,,
  \label{eq:Themis-sun-avg}
\end{equation}
where $f^\Tag{flat}_j = r^\Tag{flat}_j\,\Delta t$ is a smooth (\eg
constant) distribution and $a_j$ is the pixelwise conversion factor. In
...
Conventional calibration amounts to combining all calibration data to yield:
\begin{align}
  \estim{b}_j &= m^\Tag{dark}_j \,, \\
  \estim{g}_j &= \frac{m^\Tag{lamp}_j - m^\Tag{dark}_j}
                      {v^\Tag{lamp}_j - v^\Tag{dark}_j} \,, \\
  \estim{a}_j &= \frac{f^\Tag{flat}_j}
                      {m^\Tag{sun}_j - m^\Tag{dark}_j} \,, \\
  \estim{p}_j &= \frac{\estim{g}_j}{\estim{a}_j} \,,\\
  \estim{q}_j &= \estim{a}_j\,\estim{g}_j\,v^\Tag{dark}_j \,.
\end{align}
The estimator $\estim{b}_j$ of the bias directly comes from
Eqs.~\eqref{eq:Themis-dark-avg}. The estimator $\estim{g}_j$ of the gain
results from Eq.~\eqref{eq:Themis-lamp-var-approx} using the estimators of the
other parameters.  The estimator $\estim{a}_j$ of the intensity correction
factor results from Eq.~\eqref{eq:Themis-sun-avg} using the estimators of the
other parameters.

From Eq.~\eqref{eq:integrated-max-likelihood}, the co-log-likelihood of the
$j$-th pixel in the dark and stable images\footnote{The flat images are only
  taken into account to compute the flux correction factors $a_j$}. writes:
\begin{align}
  \varphi_j(\V\theta_j)
  &= N^\Tag{dark}\,\Paren*{
    \frac{
      v^\Tag{dark'}_j +
      \Paren*{m^\Tag{dark}_j - c^\Tag{dark}_j\,\Delta t - z_j}^2
    }{
      c^\Tag{dark}_j\,\Delta t/g_j + \sigma^2_j
    }
    + \log\Paren*{c^\Tag{dark}_j\,\Delta t/g_j + \sigma^2_j}
  } \notag\\
  &\quad+ N^\Tag{lamp}\,\Paren*{
    \frac{
      v^\Tag{lamp'}_j + \Paren*{
        m^\Tag{lamp}_j - \Paren*{c^\Tag{dark}_j + c^\Tag{lamp}_j}\,\Delta t
        - z_j
      }^2
    }{
      \Paren*{c^\Tag{dark}_j + c^\Tag{lamp}_j}\,\Delta t/g_j + \sigma^2_j
    } + \log\Paren*{
      \Paren*{c^\Tag{dark}_j + c^\Tag{lamp}_j}\,\Delta t/g_j + \sigma^2_j}
    } \notag\\
  &= g_j\,\Paren*{
    N^\Tag{dark}\,
    \frac{v^\Tag{dark'}_j + \Paren*{m^\Tag{dark}_j - b_j}^2}{x_j} +
    N^\Tag{lamp}\,
    \frac{v^\Tag{lamp'}_j + \Paren*{m^\Tag{lamp}_j - b_j - y_j}^2}{x_j + y_j}
  } \notag\\
  &\quad - \Paren*{N^\Tag{dark} + N^\Tag{lamp}}\log g_j
    + N^\Tag{dark}\,\log x_j
    + N^\Tag{lamp}\,\log\Paren*{x_j + y_j} \, ,
\end{align}
which depend on 4 unknown parameter $\V\theta_j =  (b_j, g_j, x_j, y_j)$ with:
\begin{align}
   b_j &= c^\Tag{dark}_j\,\Delta t + z_j \,,\\
   x_j &= c^\Tag{dark}_j\,\Delta t + g_j\,\sigma^2_j \,,\\
   y_j &= c^\Tag{lamp}_j\,\Delta t \,,
\end{align}
and where:
\begin{align}
  v^\Tag{dark'}_j
  &= \frac{N^\Tag{dark}}{N^\Tag{dark} - 1}\,v^\Tag{dark}_j\,,\\
  v^\Tag{lamp'}_j
  &= \frac{N^\Tag{lamp}}{N^\Tag{lamp} - 1}\,v^\Tag{lamp}_j\,,
\end{align}
denote the maximum likelihood (biased) variances.  Note that we do not attempt
to estimate the dark current $\V c^\Tag{dark}$ and the detector bias $\V z$,
only the bias $\V b = \V z + \V c^\Tag{dark}\,\Delta t$ is needed. Similarly,
we do not attempt to estimate the readout noise $\V \sigma$ separately, only
the pixelwise variance in the dark images $c^\Tag{dark}_j\,\Delta t/g_j +
\sigma^2_j$ is needed.

It appears that conventional calibration amounts to first maximize the
likelihood of the dark images in the bias $b_j$ and the noise in the dark
images and then maximize the likelihood of the LED images in the remaining
detector parameters.  The co-log-likelihood of calibration data with
conventional calibration is:
\begin{align}
  \varphi^\Tag{conv}_j =
  N^\Tag{dark}\,\Paren*{1 + \log v^\Tag{dark}_j} +
  N^\Tag{lamp}\,\Paren*{1 + \log v^\Tag{lamp}_j} - 2 \, .
\end{align}

The best bias does not depend on the gain:
\begin{align}
  b^+_j(x,y)
  &\bydef \argmin_{b} \varphi_j(b,g,x,y) \notag\\
  &= \frac{
    \frac{N^\Tag{dark}\,m^\Tag{dark}_j}{x} +
    \frac{N^\Tag{lamp}\,\Paren*{m^\Tag{lamp}_j - y}}{x + y}
  }{
    \frac{N^\Tag{dark}}{x} +
    \frac{N^\Tag{lamp}}{x + y}
  } \, .
...
\begin{align}
  g^+_j(x,y)
  &\bydef \argmin_{g > 0} \Brace*{\min_{b} \varphi_j(b,g,x,y)} \notag\\
  &= \frac{
    N^\Tag{dark} + N^\Tag{lamp}
  }{
    N^\Tag{dark}\,\frac{
      v^\Tag{dark'}_j + \Paren*{m^\Tag{dark}_j - b^+_j(x,y)}^2
    }{
      x
    } + N^\Tag{lamp}\,\frac{
      v^\Tag{lamp'}_j + \Paren*{m^\Tag{lamp}_j - b^+_j(x,y) - y}^2
    }{
      x + y
    }
  } \, .
...
\begin{align}
  \varphi^+_j(x, y)
  &\bydef \min_{b,g>0} \varphi_j(b,g,x,y) \notag \\
  &= \varphi_j\Paren*{b^+_j(x,y),g^+_j(x,y),x,y} \notag \\
  &= N^\Tag{dark}\,\Paren*{1 + \log\Paren*{\frac{x}{g_j^+(x,y)}}}
   + N^\Tag{lamp}\,\Paren*{1 + \log\Paren*{\frac{x + y}{g_j^+(x,y)}}} \, ,
\end{align}
and we minimize this function in $(x,y)$ under the constraints $x>0$ and $y≥0$.
To apply VMLM-B, the following partial derivatives are needed:
\begin{align}
  \PDer{\varphi^+_j(x, y)}{x}
  &= \frac{N^\Tag{dark}}{x} - N^\Tag{dark}\,g^ +_j(x,y)\,\frac{
      v^\Tag{dark'}_j + \Paren*{m^\Tag{dark}_j - b^+_j(x,y)}^2
    }{
      x^2
    } \notag\\
  &\quad + \frac{N^\Tag{lamp}}{x + y} - N^\Tag{lamp}\,g^+_j(x,y)\,\frac{
      v^\Tag{lamp'}_j + \Paren*{m^\Tag{lamp}_j - b^+_j(x,y) - y}^2
    }{
      \Paren*{x + y}^2
    }
    \, , \\
  \PDer{\varphi^+_j(x, y)}{y}
  &= 2\,N^\Tag{lamp}\,g^+_j(x,y)\,\frac{
       b^+_j(x,y) + y - m^\Tag{lamp}_j
    }{
      x + y
    } \notag\\
  &\quad + \frac{N^\Tag{lamp}}{x + y} - N^\Tag{lamp}\,g^+_j(x,y)\,\frac{
...
\oops{Alternatively, one can use BOBYQA.}
...
\begin{tabular}{lll}
  ...
  \FitsKeyword{OFF1}     & by default: \FitsConstant{0} & Horizontal offset \\
  \FitsKeyword{OFF2}     & by default: \FitsConstant{0} & Vertical offset \\
  \FitsKeyword{BIN1}     & by default: \FitsConstant{1} & Horizontal binning \\
  \FitsKeyword{BIN2}     & by default: \FitsConstant{1} & Vertical binning \\
  \hline
\end{tabular}
\end{center}
...
\bibliography{detector-calibration}
