...
\item the standard deviation $\sigma$ of the \emph{readout noise} (in fact, of
all other sources of noise than shot noise);
\end{itemize}
...
Available calibration data consists in sequences of images of $N_\Tag{pix}$
pixels acquired under various conditions of illumination and with different
(known) exposure times. We denote by $\V c_\ell \in \Reals^{N_\Tag{pix}}$ the
current term corresponding to a given illumination and by $\Delta t_{k,\ell}$
the different exposure times of the calibration images acquired under the
$\ell$-th illumination. There may be several images acquired under the same
conditions (same illumination and exposure time) and we denote by $\V
d_{k,\ell,n} \in \Reals^{N_\Tag{pix}}$ the $n$-th calibration image acquired
under the $\ell$-th illumination and with exposure time $\Delta t_{k,\ell}$,
let $N_{k,\ell}$ denote the number of such images.

All unknown parameters (the zero level, the gain, the currents and the
readout-noise) are pixel-wise quantities.  Assuming pixels have independent
statistics, the unknown parameters can be estimated separately for each pixel.
Unless otherwise specified, we consider a given pixel in what follows.  Above,
boldface letters were used to denote \emph{vectors} of size $N_\Tag{pix}$.
\oops{Not clear!}

Maximum likelihood estimators are well known for their good properties and our
objective is to design a numerical method to find the maximum likelihood
estimator defined by:
\begin{equation}
  \estim{\V\theta} = \argmin_{\V\theta\:\in\:\Omega}\phi(\V\theta)\,,
\end{equation}
where $\phi(\V\theta)$ is the co-log-likelihood of the available data given the
unknown parameters $\V\theta$ and $\Omega$ denotes the set of acceptable
parameters.  For a given pixel, the parameters to retrieve are $(z, g, \sigma^2,
c_{1}, c_{2}, \ldots)$ and $\V\theta$ can be any one-to-one mapping of these.

For a given pixel, the available calibration data is a set of values
$d_{k,\ell,n}$. Ignoring saturations or other defects and assuming independent
noise with a distribution approximately Gaussian, the co-log-likelihood of these
data writes:
\begin{equation}
  \label{eq:cologlikelihood}
  \phi(\V\theta) = \sum_{k,\ell,n}\Paren*{
    \frac{\Paren*{d_{k,\ell,n} -
        m_{k,\ell}}^2}{v_{k,\ell}}
    + \log v_{k,\ell}
  }
\end{equation}
with $m_{k,\ell}$ and $v_{k,\ell}$ the expectation and variance of the data
$d_{k,\ell}$ knowing the parameters $\V\theta$:
\begin{align}
  \label{eq:expectation-model}
  m_{k,\ell}
  &= \Expect{d_{k,\ell,n} \Given \V\theta}
   = c_{\ell}\,\Delta t_{k,\ell} + z\,,\\
  \label{eq:variance-model}
  v_{k,\ell}
  &= \Var{d_{k,\ell,n} \Given \V\theta}
   = c_{\ell}\,\Delta t_{k,\ell}/g + \sigma^2\,,
\end{align}
which follow from Eqs.~\eqref{eq:assumed-expectation} and
\eqref{eq:assumed-variance}.

Introducing the empirical mean and variance of the considered pixel in the
$N_{k,\ell}$ data frames acquired with the same exposure time and illumination:
\begin{align}
...
$\phi(\V\theta)$ defined in Eq.~\eqref{eq:factorized-cologlikelihood} can
significantly reduce the computational burden and the memory requirements for
minimizing $\phi(\V\theta)$ in the parameters $\V\theta$.

Note that the empirical variance $\empirical{v}_{k,\ell}$ defined by
Eq.~\eqref{eq:sample-variance} is a biased estimator of the variance\footnote{by
a factor $N_{k,\ell}/(N_{k,\ell} - 1)$}.  Also note that, if only a single data
frame has been acquired under $\ell$-th illumination and with exposure time
$\Delta t_{k,\ell}$, then $N_{k,\ell} = 1$ and $\empirical{m}_{k,\ell} =
d_{k,\ell,1}$ while $\empirical{v}_{k,\ell} = 0$.
...
assuming rounding to the nearest ADU yields $\sigma ≥ 1/\sqrt{12}$.  Other
constraints are that the incident light level and the dark current are
necessarily nonnegative and  that the gain is strictly positive.  Hence the set
of acceptable parameters is:
\begin{equation}
  \label{eq:feasible-set}
  \Omega = \Brace*{\V\theta \SuchThat \sigma ≥ 12^{-1/2},\ c_{\ell} ≥ 0
  \ (\forall\ell),\ g > 0}\,.
\end{equation}

In spite of the fact that it is separable in pixels, the maximum likelihood
criterion $\phi(\V\theta)$ is not trivial to jointly minimize in all the
parameters accounting for the constraints implemented by $\Omega$.  We propose
in the following a strategy to solve the problem by means of hierarchical
optimization and first show how to eliminate the bias and the gain thanks to a
...
necessarily strictly positive, the readout noise variance is retrieved by
$\sigma^2 = \vartheta/g$ so we can choose the unknown parameters to be:
\begin{equation}
  \label{eq:unknowns}
  \V\theta = (z, g, \vartheta, c_{1}, c_{2}, \ldots) \, .
\end{equation}
...
A first interesting consequence of the chosen parametrization is that the
...
\begin{align}
  z^+\Paren*{\V\theta\setminus\Brace{z,g}}
  &\bydef \argmin_{z} \phi(\V\theta) \notag\\
  &= \argmin_{z} g\,\sum_{k,\ell} N_{k,\ell}\,\frac{
     \empirical{v}_{k,\ell} + \Paren*{\empirical{m}_{k,\ell} - m_{k,\ell}}^2
   }{
     u_{k,\ell}
   } \notag \\
  &= \argmin_{z} \sum_{k,\ell} \frac{
     N_{k,\ell}\,\Paren*{\empirical{m}_{k,\ell} - m_{k,\ell}}^2
   }{
     u_{k,\ell}
   } \notag \\
  &= \frac{
      \sum_{k,\ell} N_{k,\ell}\,
      \Paren*{\empirical{m}_{k,\ell} - c_\ell\,\Delta t_{k,\ell}}/u_{k,\ell}
    }{
      \sum_{k,\ell} N_{k,\ell}/u_{k,\ell}
    }
\end{align}
which follow from the fact that $m_{k,\ell}$ linearly depends on $z$ according
to Eq.~\eqref{eq:expectation-model} while $u_{k,\ell}$ does not depend on $z$,
and that the gain $g$ is strictly positive.  The expression
$\V\theta\setminus\Brace{z,g}$ denotes the set of parameters $\V\theta$ minus
the parameters $z$ and $g$.

Noting that $m_{k,\ell}$ and $u_{k,\ell}$ do not depend on the gain $g$ and that
$\phi(\V\theta)$ is strictly convex in $g$ (for $g > 0$), the best gain knowing
the other parameters has a closed form expression given by:
\begin{equation}
  \label{eq:best-gain-knowing-others}
  \argmin_{g} \phi(\V\theta)
  = \frac{\sum_{k,\ell} N_{k,\ell}}{
    \sum_{k,\ell} N_{k,\ell}\,\Paren*{
      \empirical{v}_{k,\ell}
...
\end{equation}
which is obtained by solving $\partial\phi(\V\theta)/\partial g=0$ in $g$.  The
right hand side of Eq.~\eqref{eq:best-gain-knowing-others} is always strictly
positive.  Hence it is also the minimum of $\phi(\V\theta)$ in $g$ under the
constraint that $g > 0$. Now since $z^+$, the minimum of $\phi(\V\theta)$ in $z$
does not depend on the gain $g$, replacing the zero level $z$ by $z^+$ in the
right hand side of Eq.~\eqref{eq:best-gain-knowing-others} yields the maximum
likelihood estimator of the gain knowing all other parameters than $z$ and $g$:
\begin{align}
  g^+\Paren*{\V\theta\setminus\Brace{z,g}}
  &\bydef \argmin_{g > 0} \Brace*{\min_{z} \phi(\V\theta)} \notag\\
  &= \frac{\sum_{k,\ell} N_{k,\ell}}{
    \sum_{k,\ell} N_{k,\ell}\,\Paren*{
      \empirical{v}_{k,\ell}
...
\begin{align}
  ...
  \psi(\V\theta\setminus\Brace{z,g})
  &\bydef \min_{z,g > 0}\phi(\V\theta) \notag\\
  &= \sum_{k,\ell} N_{k,\ell}\,\log u_{k,\ell}
     + N\,\Paren*{1 - \log g^+} \notag \\
  &= \xi + \sum_{k,\ell} N_{k,\ell}\,\log u_{k,\ell} \notag \\
  &\quad + N\,\log\Paren[\Bigg]{\sum_{k,\ell} \frac{N_{k,\ell}}{u_{k,\ell}}\,
    \Paren*{\empirical{v}_{k,\ell} +
    \Paren[\big]{\empirical{m}_{k,\ell} - m^+_{k,\ell}}^2}}
  \, \,
\end{align}
with $N = \sum_{k,\ell}N_{k,\ell}$ is the total number of data and $\xi = N - N
\,\log N$ is a constant.

\subsection{Hierarchical Optimization}

With the proposed change of variables, $\vartheta = g\,\sigma^2$, the
constraint on the
readout noise variance becomes $\vartheta ≥ g\,\sigma^2_\Tag{min}$ and the
standard
deviation of the readout noise in ADU/pixel/frame is given by $\sigma =
\sqrt{\vartheta/g}$. Provided the constraints $\vartheta > 0$ and $c_{\ell} ≥
0$ hold, then
$c_{\ell}\,\Delta t_{k,\ell} + \vartheta > 0$ and the right hand side of
Eq.~\eqref{eq:best-gain} is strictly positive\footnote{Unless $d_{k,\ell} =
c_{\ell}\,\Delta t_{k,\ell} + z^+(\V c, \vartheta)$ whatever $k$ and $\ell$
which
implies exact model and noiseless data!}.

%with:
%\begin{equation}
%  w_{k,\ell}(\V c, \vartheta) =
%  1/\Paren*{ c_{\ell}\,\Delta t_{k,\ell} + \vartheta} \, ,
%\end{equation}
...
\begin{align}
  \Paren[\big]{\estim{\V c}, \estim{\vartheta}}
  &= \argmin_{\V c ≥ 0, \vartheta > 0}
  \Brace[\Big]{\min_{g,z} \phi(\V\theta)} \notag \\
  &= \argmin_{\V c ≥ 0, \vartheta > 0} \psi(\V c, \vartheta)
\end{align}

\paragraph{Non-linear optimization} There is no closed form for the minimum of
$\psi(\V c, \vartheta)$, but starting from some initial guess $\V c\Iter0$ and
$\vartheta\Iter0$, $\psi(\V c, \vartheta)$ can be minimized by a nonlinear
multi-dimensional
...
(divided by the gain):
\begin{align}
  x_{k,\ell}(\V c, \vartheta)
  &= d_{k,\ell} - c_{\ell}\,\Delta t_{k,\ell} - z^+(\V c, \vartheta) \,, \\
  w_{k,\ell}(\V c, \vartheta)
  &= \Paren*{c_{\ell}\,\Delta t_{k,\ell} + \vartheta}^{-1}\,,
\end{align}
so that:
\begin{equation}
  \psi(\V c, \vartheta) = \sum_{k,\ell} \Paren*{
    g^+\,w_{k,\ell}\,x_{k,\ell}^2 - \log g^+ - \log w_{k,\ell}
 } \, ,
\end{equation}
where the dependency of all the terms of the right hand side with $\V c$ and
$\vartheta$ is implicitly assumed to simplify the expression. The partial
derivatives
of the criterion are needed by VMLMB. These derivatives are given by:
\begin{align}
  \PDer{\psi(\V c, \vartheta)}{c_{\ell}}
  &=\sum_{k} w_{k,\ell}\cdot\Paren*{
    1 - g^+ \cdot x_{k,\ell}\cdot\Paren*{2 + w_{k,\ell}\cdot x_{k,\ell}}
  }\cdot\Delta t_{k,\ell} \,,\\
  \PDer{\psi(\V c, \vartheta)}{\vartheta}
...
\vartheta)$ and $g^+(\V c, \vartheta)$ minimize $\phi(\V\theta)$ in $z$ and
$g$.
...
\begin{align}
  \PDer{^2\psi(\V c, \vartheta)}{c_{\ell}^2}
  &= 2\,g^+\,\sum_{k} w_{k,\ell}\,
  ...
  %
  \PDer{^2\psi(\V c, \vartheta)}{\vartheta^2}
  &= 2\,g^+\,\sum_{k,\ell} w_{k,\ell}^3\,x_{k,\ell}^2
   - \sum_{k,\ell} w_{k,\ell}^2
  \notag\\
  &\quad - \PDer{g^+}{\vartheta}\,\sum_{k,\ell} w_{k,\ell}^2\,x_{k,\ell}^2
  \notag\\
  &\quad+ 2\,g^+\,\sum_{k,\ell} w_{k,\ell}^2\,x_{k,\ell}\,\PDer{z^+}{\vartheta}
  \notag\\
...
averaged values:
\begin{align*}
  \Avg[\big]{w_{k,\ell}(\V c, \vartheta)\,x_{k,\ell}(\V c, \vartheta)}_{k}
  &\approx 0 \,,\\
  g^+(\V c, \vartheta)\,\Avg[\big]{w_{k,\ell}(\V c, \vartheta)\,
    x_{\ell}^2(\V c, \vartheta)}_{k,\ell}
  &= 1\,,
\end{align*}
...
%  }
%\end{equation}
%with, using the parameter $\vartheta$ instead of $\sigma$, $\V\theta =
%(b,\Paren*{c_{\ell}}_{\ell=1,2,\ldots},g,\vartheta)$, $\Omega$ the feasible
%set of
...
%  \\
%  v_{k,\ell}(\V\theta)
%  &= \Paren*{c_{\ell}\,\Delta t_{k,\ell} + \vartheta}/g \,.
%\end{align}
%The feasible set $\Omega$ accounts for constraints on the parameters:
%\begin{equation}
%  \Omega = \Brace*{
%    \V\theta = (b,\Paren*{c_{\ell}}_{\ell=1,2,\ldots},g,\vartheta) \in
%    \Reals^{3+n_\Tag{cls}} \SuchThat
%    \forall\ell, c_\ell ≥ 0, g > 0, \vartheta ≥ g/12
%  } \, .
%\end{equation}
...
and the ratio $\tau_{\ell} = \vartheta/c_{\ell} > 0$ are assumed known, the
...
\begin{equation}
  ...
  \label{eq:initial-c}
\end{equation}
To select the initial value $\vartheta\Iter0$ of $\vartheta$, we assume that
the readout
